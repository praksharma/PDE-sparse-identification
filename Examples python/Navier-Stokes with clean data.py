...
for p in points.keys():
    ...
    wt[p] = PolyDiffPoint(Wn[int(x),int(y),int(t-max(boundary, boundary_x)/2):int(t+max(boundary, boundary_x)/2)], np.arange(max(boundary, boundary_x))*dt, deg, 1)[0] # see line 127
