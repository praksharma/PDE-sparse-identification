...
def read_vtu_and_convert_to_structured(path, vtu_num, limits):
    ...
    U = np.zeros((num_y,num_x,vtu_stop-vtu_start))
    V = np.zeros_like(U)
    P = np.zeros_like(U)
    ...
    for n in range(0, vtu_stop-vtu_start):
        print('File: ', n+1,' of ',vtu_stop-vtu_start)
...
def exporting_dxdydt(path):
    # computing dx and dy
    coordinates_meshgrid = np.load('data/coordinates_data.npy')
    # The axis-3 stores meshgrids of x and y in index 1 and 2 respectively.
    dx = coordinates_meshgrid[0,1,0] - coordinates_meshgrid[0,0,0] # X changes along column i.e. axis 2
    dy = coordinates_meshgrid[1,0,1] - coordinates_meshgrid[0,0,1] # Y changes along rows i.e. axis 1
    # Rounding to 4 digits after decimal
    dx = np.round(dx, 4)
    dy = np.round(dy, 4)
    # computing the dt
    file_one = path + "/circle-2d-drag_" + str(0)+ ".vtu" # name of 1st vtu files
    file_end = path + "/circle-2d-drag_" + str(1)+ ".vtu" # name of 2nd vtu files
    mesh = meshio.read(file_one)
    time_one = mesh.point_data['Time'][0] # we need any index since, the time is same for all nodes in same timestep(vtu file)
    mesh = meshio.read(file_end)
    time_two = mesh.point_data['Time'][0]
    dt = time_two-time_one
    
    steps = np.array([dx, dy, dt])
    # saving the data to a file
    np.save('data/steps.npy', steps)